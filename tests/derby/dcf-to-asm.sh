...
function compile {
  ...
  time "$RUNNER" --target=assembly           "$1" -o "$ASM_DIR/$FILENAME.unoptimized.s" >&2
  time "$RUNNER" --target=assembly --opt=all "$1" -o "$ASM_DIR/$FILENAME.optimized.s" >&2
...
mkdir -p "$ROOT/tests/derby/"{asm,bin,output}
...
if [[ ${#1} -eq 0 ]]; then  # no args provided
  ...
  "$ROOT/tests/parallel" "compile {} '$ROOT/tests/derby/asm'"
